  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const stream = vedaService.chatStream(history, {
        ...
        phi: data?.phi,
        status: data?.status
      }, { name: isArchitect ? 'Architect' : 'User' }, isArchitect);
    ...
    <div className="h-full flex flex-col pt-24 md:pt-32 pb-24 md:pb-8 px-4 md:px-6 lg:px-32 max-w-6xl mx-auto relative">
      <div className="absolute top-8 right-8 z-[1001] flex gap-2">
         <button 
           ...
           className={cn(
             ...
             lang === 'zh-TW' ? "bg-accent text-black border-accent" : "text-white/40 border-white/10 hover:text-white"
           )}
         >
           ZH-TW
         </button>
         <button 
           ...
           className={cn(
             "px-2 py-1 text-[8px] font-mono border transition-all",
             lang === 'en-US' ? "bg-accent text-black border-accent" : "text-white/40 border-white/10 hover:text-white"
           )}
         >
           EN-US
         </button>
      ...
      {messages.length > 0 && (
         <motion.button
           ...
           className="absolute top-28 md:top-36 right-8 md:right-12 z-50 p-2 bg-red-500/10 border border-red-500/20 rounded-full text-red-400/60 hover:bg-red-500/20 hover:text-red-400 transition-all flex items-center gap-2 px-3 shadow-xl backdrop-blur-md group"
      ...
      >
        <AnimatePresence initial={false}>
          {messages.length === 0 && (
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="h-full flex flex-col items-center justify-center gap-6 italic text-white/5"
            >
               <VedaCrystalLogo size={60} className="opacity-20 translate-y-12" />
                 ...
                 msg.role === 'user' ? "user-bubble" : "ai-bubble"
                  ...
                  {msg.imageUrl && (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      className="mt-4 rounded-lg overflow-hidden border border-white/10 relative group"
                    >
                      ...
                      <button 
                        ...
                        className="absolute top-2 right-2 p-2 bg-black/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-accent hover:text-black shadow-lg"
                      >
                         <Download size={14} />
                      </button>
                    </motion.div>
                  ...
                  {msg.videoUrl && (
                    ...
                    >
                      ...
                      <button 
                         ...
                         className="absolute top-2 right-2 p-2 bg-black/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-accent hover:text-black z-10"
                      >
                         <Download size={14} />
                      </button>
                    </motion.div>
                  ...
                  {msg.audioUrl && (
                    <motion.div 
                      ...
                      className="mt-4 p-3 bg-accent/10 border border-accent/20 rounded-lg flex items-center gap-3"
                    >
                      <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center text-black">
                        <Activity size={14} className="animate-pulse" />
                      </div>
                      <audio src={msg.audioUrl} controls className="flex-1 h-8" />
                    </motion.div>
                  )}
                  ...
                  {(msg.text?.includes("REPORT_") || msg.actions?.some(a => a.name === 'initiateStrategicReport' || a.name === 'ACTION_RESULTS')) && data?.strategic_reports && data.strategic_reports.length > 0 && (
                   <motion.div 
                     initial={{ opacity: 0, y: 10 }}
                     animate={{ opacity: 1, y: 0 }}
                     className="mt-4 p-6 bg-accent/5 border border-accent/20 rounded-xl flex flex-col gap-4 shadow-2xl"
                   >
                            ...
                            <div key={report.id} className="flex flex-col gap-4">
                               <div className="flex justify-between items-end">
                                  <div className="flex flex-col">
                                     <span className="text-[10px] font-mono opacity-40 uppercase tracking-tighter">Document Title</span>
                                     <span className="text-sm font-bold tracking-wide">{report.title}</span>
                                  </div>
                                  <span className="text-[10px] font-mono text-accent">{report.progress.toFixed(0)}%</span>
                               </div>
                               <button 
                                 onClick={() => exportReportToPDF(report)}
                                 className="w-full py-3 bg-accent/10 border border-accent/30 text-accent text-[9px] font-bold tracking-[0.3em] uppercase hover:bg-accent/20 transition-all flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(var(--accent-rgb),0.2)]"
                               >
                                 <Download size={14} />
                 ...
                 <div className="px-6 flex items-center gap-6 mt-1">
                    {msg.mode && (
                       <div className="flex items-center gap-2">
                          <div className={cn(
                            ...
                            msg.mode === 'LOCAL' ? "bg-gold shadow-[0_0_5px_#fbbf24]" : "bg-white/20"
                          )} />
                          <span className="text-[7px] tracking-[0.3em] font-display text-white/30 uppercase">
                            {msg.mode} ENGINE
                          </span>
                       </div>
                    )}
                    {msg.confidence !== undefined && (
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[7px] font-mono tracking-widest text-white/30 italic">CON_LEVEL: {(msg.confidence * 100).toFixed(0)}%</span>
                      </div>
                    )}
                 </div>
               )}
                        ...
                        <div className="mt-4 p-4 ghibli-glass border border-accent/20 rounded-xl flex flex-col gap-4 animate-in fade-in slide-in-from-top-2">
                           ...
                           <div className="flex flex-col gap-2">
                              {action.actions.map((a: any, i: number) => (
                                 <div key={i} className="text-[9px] font-mono text-white/60 bg-white/5 p-2 rounded">
                           ...
                           <div className="flex gap-4">
                              <button 
                                ...
                                className="flex-1 py-1.5 bg-accent/20 border border-accent/40 text-accent text-[10px] font-mono hover:bg-accent hover:text-black transition-all uppercase tracking-widest"
                              >
                                APPROVE_PROCEED
                              </button>
                              <button 
                                ...
                                className="px-4 py-1.5 border border-white/10 text-white/40 text-[10px] font-mono hover:border-red-400 hover:text-red-400 transition-all uppercase"
                              >
                                REJECT
                              </button>
                           </div>
       ...
       <div className="mt-8 group relative max-w-4xl mx-auto w-full">
        ...
        <div className="ghibli-glass border border-white/10 rounded-[2rem] p-2 shadow-2xl transition-all duration-1000 focus-within:border-accent/40 bg-zinc-900/80">
            <div className="flex items-center px-4">
                 <label className="p-3 text-white/20 hover:text-accent cursor-pointer transition-all">
                        ...
                        if (file) {
                          ...
                          reader.onload = async (event) => {
                            ...
                            const type = file.type.startsWith('image/') ? 'IMAGE' : file.type.startsWith('video/') ? 'VIDEO' : 'AUDIO';
                             await onAction('registerVisualAsset', { type, url: dataUrl, prompt: `Manual upload: ${file.name}` });
                             await onUpdateData();
                          };
                ...
                <textarea
                  ...
                  className="flex-1 bg-transparent border-none p-4 text-base md:text-lg font-sans outline-none resize-none h-14 md:h-20 scrollbar-none placeholder:text-white/10 text-white/80"
                />
                <button
                   onClick={sendMessage}
